...
| `reader-type.html` | Case study: Reader Type. Links straight to `/reader-type/`, no embed wrapper (see 5. "Storage-dependent apps") |
| `reader-type/` | Copy of the Reader Type app, served same-origin at `/reader-type/`. No build step: it is plain HTML, CSS, and JS copied from the `online-quiz` repo. Refresh by copying `index.html`, `app.js`, `data.js`, `styles.css` over. It carries its own stylesheet but uses this site's tokens, fonts, radius and `theme` storage key, so it should look continuous with the rest of the site. If the site palette changes, that copy needs the same edit by hand. |
| `poem-type.html` | Case study: Poem Type. Links straight to `/poem-type/`, no embed wrapper (see 5. "Storage-dependent apps") |
| `poem-type/` | Copy of the Poem Type app, served same-origin at `/poem-type/`. Same arrangement as `reader-type/`: no build step, plain HTML, CSS and JS copied from the same `online-quiz` repo, where it lives in a `poem-type/` folder beside `reader-type/`. Refresh by copying `index.html`, `app.js`, `data.js`, `styles.css` over. It carries its own stylesheet using this site's tokens, so a palette change needs the same edit by hand. |
...
("View project") to the case study. Estoria is pinned first by Ray's request.
The bottom is a reserved slot holding the two quizzes as a pair, Reader Type
then Poem Type, in the order they were built and by Ray's request; everything
between them is newest first. A new project goes directly below Estoria unless
told otherwise, not at the very bottom.
...
Reader Type (2026-08-22) is the third direct link, under the storage half of
that rule. It keeps two things in `localStorage`: the theme choice, and the
feedback responses a reader leaves when they say whether the result was right.
The second is the whole point of the feature, since those responses are what
would eventually let the genre model be fitted to real data instead of authored
by hand, so having them land in a partitioned third-party store would quietly
lose them. Same-origin, no wrapper, same as Pace.

Poem Type (2026-08-27) is the fourth, for exactly the same reason and with the
same shape: theme plus feedback responses in `localStorage`, no wrapper, served
same-origin from `/poem-type/`.
...
| `reader-type` | Reader Type | https://www.labrarf.com/reader-type/ (same-origin copy, the only live URL; the source repo `labrarf-rgb/online-quiz` is private) | No |
| `poem-type` | Poem Type | https://www.labrarf.com/poem-type/ (same-origin copy, the only live URL; shares the private `labrarf-rgb/online-quiz` source repo with Reader Type) | No |

All seven appear on `projects.html`. Featured on the home page is currently
Estoria and UnifyCRM only. Reader Type and Poem Type sit deliberately last as a
pair, in that order, and neither is featured.
...
### 2026-08-22 - Added Reader Type; linked direct; pinned last and not featured

- **Added the Reader Type project.** New `reader-type.html` (case study) and a
  same-origin copy of the app under `reader-type/`, served at
  `www.labrarf.com/reader-type/`. A book quiz that profiles the reader across
  ten dimensions from twenty four agree-scale statements and matches that
  against sixteen genres.
- **Placement was specified, not inferred.** Ray asked for it **last** in the
  `projects.html` list rather than first, and **not** in the home page featured
  grid. So the 2-up grid stays Estoria + UnifyCRM, untouched, and nothing was
  demoted. Recorded in section 5 that the bottom of the list is now a pinned
  slot, so future projects go below Estoria rather than at the very end.
- **No `reader-type-app.html` was created,** under the storage half of the
  direct-link rule. The app keeps the reader's theme and, more importantly,
  their feedback responses in `localStorage`. Those responses are the mechanism
  by which the genre model could eventually be fitted to real data rather than
  hand-authored, so losing them to a partitioned third-party store would defeat
  the feature. Same fix as Pace.
- **No build step.** Unlike Estoria and Pace, Reader Type is plain HTML, CSS,
  and JS, so `reader-type/` is a straight copy of four files rather than build
  output. Refresh it by copying `index.html`, `app.js`, `data.js`, and
  `styles.css` from the source repo.
- **Source repo is private** (`labrarf-rgb/online-quiz`, named `reader-type` at
  the time of this entry), which is fine because
  the live copy is served from here. GitHub Pages is not enabled on it and does
  not need to be.
- Verified before pushing: both link directions resolve, the case study loads
...
### 2026-08-23 - Reader Type adopts the site's design tokens

- **The app was styled as its own thing** and it showed on click-through from
  the case study: a different palette (the warmer Estoria cream rather than this
  site's), Spectral and Hanken Grotesk rather than Raleway and Nunito, a 4px
  radius against this site's 2px, and its own `readertype-theme` storage key so
  the site's dark choice did not carry into it.
- **Now uses this site's tokens** in its own stylesheet, including the `theme`
  key, with a one-time migration for anyone who had set the old one. Contrast
  re-checked: every text and surface pair clears 4.5:1 in both themes, worst
  case 4.69 light and 4.89 dark.
- **Caveat worth remembering:** `reader-type/styles.css` is a copy, so it does
  not inherit changes to this site's `styles.css`. Same situation as
  `contact.html`. If the palette moves, move it in both places.
- **Also fixed a grid hole.** Three books were laid out in two columns, so the
  third sat alone beside an empty cell that read as a missing card. Three
  columns now fill the row exactly.

### 2026-08-27 - Added Poem Type

- **Added the Poem Type project.** New `poem-type.html` (case study) and
  `poem-type/` (same-origin copy of the app). Fourth project to be linked
  directly rather than wrapped in an `*-app.html` iframe, under the storage
  half of the rule in section 5: it keeps the theme choice and the reader's
  feedback responses in `localStorage`, and the responses are the whole point
  of that feature, so a partitioned third-party store would lose them.
- **Placed last on `projects.html`, after Reader Type**, at Ray's request. The
  reserved bottom slot now holds both quizzes as a pair rather than Reader Type
  alone. Section 5 updated to say so.
- **Not featured on the home page**, at Ray's request. `index.html` is
  untouched by this change and the featured grid stays at Estoria and UnifyCRM.
- **Case study modelled on `reader-type.html`**, at Ray's request, so the two
  quizzes read as siblings: same element order, same tag shape (`Prototype`,
  `Poetry`, `Web App`, `Vanilla JS`), same closing caveat structure.
- **Source repo shape changed.** The private `labrarf-rgb/online-quiz` repo now
  holds two apps, `reader-type/` and `poem-type/`, rather than one at its root.
  Both folders here are copied from their matching folder there.
- **Known, pre-existing, not fixed here:** `script.js` line 25 assumes
  `#contact-form` exists, so every case study page throws a TypeError in the
  console. Nothing runs after it in that file, so the theme toggle (registered
  earlier) still works and the pages are unaffected. It predates this change
  and is logged rather than fixed, to keep this commit to one project.

### 2026-08-27 - Source repo renamed

- **`labrarf-rgb/reader-type` is now `labrarf-rgb/online-quiz`.** It holds two
  apps (`reader-type/` and `poem-type/`), so naming it after one of them was
