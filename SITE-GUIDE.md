...
("View project") to the case study. Estoria is pinned first and Reader Type is
pinned last, both by Ray's request; everything between them is newest first.
A new project goes directly below Estoria unless told otherwise, not at the
very bottom, which is now a reserved slot.
...
All six appear on `projects.html`. Featured on the home page is currently
Estoria and UnifyCRM only. Reader Type is deliberately last in the list and
