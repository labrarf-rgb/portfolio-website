...
| `reader-type/` | Copy of the Reader Type app, served same-origin at `/reader-type/`. No build step: it is plain HTML, CSS, and JS copied from the `reader-type` repo. Refresh by copying `index.html`, `app.js`, `data.js`, `styles.css` over. It carries its own stylesheet but uses this site's tokens, fonts, radius and `theme` storage key, so it should look continuous with the rest of the site. If the site palette changes, that copy needs the same edit by hand. |
| `poem-type.html` | Case study: Poem Type. Links straight to `/poem-type/`, no embed wrapper (see 5. "Storage-dependent apps") |
| `poem-type/` | Copy of the Poem Type app, served same-origin at `/poem-type/`. Same arrangement as `reader-type/`: no build step, plain HTML, CSS and JS copied from the same `reader-type` repo, where it lives in a `poem-type/` folder beside `reader-type/`. Refresh by copying `index.html`, `app.js`, `data.js`, `styles.css` over. It carries its own stylesheet using this site's tokens, so a palette change needs the same edit by hand. |
...
| `reader-type` | Reader Type | https://www.labrarf.com/reader-type/ (same-origin copy, the only live URL; the source repo `labrarf-rgb/reader-type` is private) | No |
| `poem-type` | Poem Type | https://www.labrarf.com/poem-type/ (same-origin copy, the only live URL; shares the private `labrarf-rgb/reader-type` source repo with Reader Type) | No |
...
- **Source repo is private** (`labrarf-rgb/reader-type`), which is fine because
...
- **Source repo shape changed.** The private `labrarf-rgb/reader-type` repo now
